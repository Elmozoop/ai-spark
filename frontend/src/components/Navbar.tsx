    ...
    <nav className="navbar-enter fixed top-0 w-full z-50 border-b border-stone-200 bg-white/80 backdrop-blur-xl">
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-6 animate-fade-in delay-500">
          ...
          <div className="flex items-center gap-1.5 px-2 py-1 rounded-full bg-stone-100 border border-stone-200 text-[10px] font-bold text-stone-500">
             <button 
               onClick={() => setLanguage('en')}
               className={`px-2 py-0.5 rounded-full transition-all ${language === 'en' ? 'bg-orange-500 text-white' : 'hover:text-stone-900'}`}
             >
               EN
             </button>
             <button 
               onClick={() => setLanguage('hi')}
               className={`px-2 py-0.5 rounded-full transition-all ${language === 'hi' ? 'bg-green-600 text-white' : 'hover:text-stone-900'}`}
             >
               हि
             </button>
          </div>
