...
const statusConfig = {
  Genuine: { color: 'text-green-700', bg: 'bg-green-50 border-green-200', icon: CheckCircle2 },
  Suspicious: { color: 'text-amber-700', bg: 'bg-amber-50 border-amber-200', icon: AlertTriangle },
  Forged: { color: 'text-red-700', bg: 'bg-red-50 border-red-200', icon: XCircle },
};
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 px-4 py-2.5 bg-white border border-stone-200 shadow-sm rounded-xl text-sm text-stone-500">
            <Search className="w-4 h-4" />
            <span>Search documents...</span>
          </div>
          <button className="flex items-center gap-2 px-4 py-2.5 bg-white border border-stone-200 shadow-sm rounded-xl text-sm text-stone-500 hover:text-stone-900 transition-all">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: 'Total Scanned', value: '47', sub: 'this session', color: 'text-stone-900' },
          { label: 'Auto-Approved', value: '38', sub: 'score ≤ 30', color: 'text-green-600' },
          { label: 'Rejected / Flagged', value: '9', sub: 'score > 30', color: 'text-red-600' },
        ].map((stat, i) => (
          <motion.div
            key={i}
          ...
          >
            <p className="text-xs text-stone-500 uppercase tracking-wider font-bold mb-2">{stat.label}</p>
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-stone-100">
          ...
          <span className="text-xs text-stone-500">{MOCK_DOCS.length} documents</span>
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-4">
                  <div className="w-9 h-9 rounded-lg bg-stone-50 border border-stone-200 flex items-center justify-center shadow-sm">
                    <FileText className="w-4 h-4 text-stone-500" />
                  </div>
                  <div>
                    <p className="text-sm font-medium text-stone-900">{doc.name}</p>
                    <p className="text-xs text-stone-500">{doc.type} · {doc.date}</p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  <span className="text-sm font-bold text-stone-500 tabular-nums">
                    Score: <span className={cfg.color.replace('bg-', 'text-')}>{doc.score}</span>/100
                  </span>
